...
const storage = multer.diskStorage({
  ...
  filename: function (req, file, cb) {
    ...
    const fullName = req.body.name ? req.body.name.replace(/\s+/g, '-') : 'default-name'; // Replace spaces with hyphens
    
    // Create a unique filename with full name, timestamp, and random number
    const uniqueName = `${fullName}-${Date.now()}-${Math.floor(Math.random() * 1000)}${path.extname(file.originalname)}`;
